...
def check_user(request):
    ...
    if request.method == "POST":
        ...
        result = get_object_or_404(Users, email=email)
        ...
        if result:
            ...
            elif role == "Basicuser" and status:
                return JsonResponse({"redirect": "/Basicuser"})
            elif role == "Basicuser" and not status:
                return JsonResponse({"redirect": "/Deactivated"})
...
def Administrator(request):
    ...
    users = Users.objects.all().order_by('role', 'status', 'name')
...
def create_user(request):
    if request.method == "POST":
        ...
        if not (name and email and role and status):
...
def update_user(request):
    if request.method == "POST":
        ...
        try:
            # It's safe to assume only one user will match with the email
            # since emails are meant to be unique here.
        ...
        user.name = name
        user.email = email
        print('DEBUG: ', role == "", status == "")
        if role in ['Basicuser', 'Administrator']:
            user.role = role
        if type(status) == bool:
            user.status = status
...
def deactivated_user(request):
    return render(request, 'Deactivated.html')