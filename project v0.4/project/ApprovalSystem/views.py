...
from UserManagement.models import User, Delegate, Workflow
from .models import Public_info, Early_withdrawal, Graduate_petition, Undergraduate_transfer, Historical_approval
...
def RequestMenu(request):
    ...
    if User.objects.get(email=request.session.get("user_email")).cougarID is None:
        return render(request, 'User_no_cougarID.html')
    elif request.session.get("cougarID") != 1:
        return render(request, 'User_verify_cougarID.html')
    else:
        return render(request, 'RequestMenu.html', {"user": user})
    
def UserinputCougarID(request):
    ...
    return redirect(reverse('ApprovalSystem:RequestMenu'))

def UserverifyCougarID(request):
    cougarID = request.POST.get("cougarID")
    user = User.objects.get(email=request.session.get("user_email"))
...
def submitPublicInfo(request):
    if request.method == "POST":
        ...
        # get the number of level of the form that were set by the admin
        # if the number of level is 0, the form will be no need to be approved
        # if the number of level is 1, the form will be need to be approved by the level1 approver
        # if the number of level is 2, the form will be need to be approved by the level1 and level2 approver
        num_of_level = Workflow.objects.get(form_name='Public_Info').Level
        if num_of_level == '0':
        ...
        return redirect(reverse('ApprovalSystem:PublicInformation'))
...
def submitEarlyWithdrawal(request):
    if request.method == "POST":
        ...
        # if the number of level is 1, the form will be need to be approved by the level1 approver
        # if the number of level is 2, the form will be need to be approved by the level1 and level2 approver
        num_of_level = Workflow.objects.get(form_name='Early_withdrawal').Level
        if num_of_level == '0':
            form_status = 'approved'
            form_level1 = '0'
            form_level2 = '0'
        ...
        return redirect(reverse('ApprovalSystem:EarlyWithdrawal'))
...
def get_GraduatePetition_pdf(request):
    ...
    # if the number of level is 1, the form will be need to be approved by the level1 approver
    # if the number of level is 2, the form will be need to be approved by the level1 and level2 approver
    num_of_level = Workflow.objects.get(form_name='graduate student petition form').Level
    if num_of_level == '0':
        form_status = 'approved'
        form_level1 = '0'
        form_level2 = '0'
    ...
    Graduate_petition.objects.update_or_create(
        email=user_email,
        defaults={
            'level1': form_level1,
            'level2': form_level2}
    ...
    # if form level is set to 0, which mean it doesn't neet to be approved, set the status to approved
...
def get_UndergraduateTransfer_pdf(request):
    ...
    # if the number of level is 1, the form will be need to be approved by the level1 approver
    # if the number of level is 2, the form will be need to be approved by the level1 and level2 approver
    num_of_level = Workflow.objects.get(form_name='undergraduate transfer form').Level
    if num_of_level == '0':
        form_status = 'approved'
        form_level1 = '0'
        form_level2 = '0'
    ...
    Undergraduate_transfer.objects.update_or_create(
        email=user_email,
        defaults={
            'level1': form_level1,
            'level2': form_level2}
